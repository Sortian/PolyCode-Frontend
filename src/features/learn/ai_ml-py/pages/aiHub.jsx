...
export default function AiHub() {
  ...
  const earnedXP = AI_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct = Math.round((completedCount / AI_LESSONS.length) * 100) || 0;
  const nextLesson =
    AI_LESSONS.find((lesson) => !progress[lesson.id]) || AI_LESSONS[0];
  const resumeLesson =
    AI_LESSONS.find((lesson) => lesson.id === lastLessonId) || nextLesson;
  const completedChapters = AI_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => AI_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);
  ...
  return (
    <div className="oops-hub pandas-hub">
      <div className="oops-hero pandas-hero">
        <Link
          to="/language/Python"
        ...
        <div className="oops-hero-badge">AI / ML / DL · PYTHON TRACK</div>
        ...
        <p className="oops-hero-sub">
          Learn AI foundations, Machine Learning, Deep Learning, Transformers,
          and Generative AI with hands-on Python challenges — beginner to
          advanced.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${AI_LESSONS.length} lessons · ${earnedXP}/${AI_TOTAL_XP} XP`
                  : `Sign in to track progress · ${AI_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume AI/ML" : "Start AI/ML"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find an AI/ML topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search neural network, SVM, transformer..."
              aria-label="Search AI/ML lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter AI/ML lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{AI_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{AI_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{AI_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
