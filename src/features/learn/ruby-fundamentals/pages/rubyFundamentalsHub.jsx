...
export default function RubyFundamentalsHub() {
  ...
  const earnedXP = RUBY_FUNDAMENTALS_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / RUBY_FUNDAMENTALS_LESSONS.length) * 100) || 0;
  const nextLesson =
    RUBY_FUNDAMENTALS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    RUBY_FUNDAMENTALS_LESSONS[0];
  const resumeLesson =
    RUBY_FUNDAMENTALS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = RUBY_FUNDAMENTALS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => RUBY_FUNDAMENTALS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return RUBY_FUNDAMENTALS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub ruby-fundamentals-hub">
      <div className="oops-hero ruby-fundamentals-hero">
        <Link
          to="/language/Ruby"
        ...
        <div className="oops-hero-badge">Ruby · CORE TRACK</div>
        ...
        <p className="oops-hero-sub">
          From your first `puts` to blocks, hashes, pure OOP, and mixins —
          simple theory, real-life examples, diagrams, and hands-on Ruby
          challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${RUBY_FUNDAMENTALS_LESSONS.length} lessons · ${earnedXP}/${RUBY_FUNDAMENTALS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${RUBY_FUNDAMENTALS_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume Ruby" : "Start Ruby"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a Ruby topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search hashes, blocks, mixins..."
              aria-label="Search Ruby lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter Ruby lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{RUBY_FUNDAMENTALS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{RUBY_FUNDAMENTALS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{RUBY_FUNDAMENTALS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
