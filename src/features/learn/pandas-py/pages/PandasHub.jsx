...
export default function PandasHub() {
  ...
  const resumeLesson =
    PANDAS_LESSONS.find((lesson) => lesson.id === lastLessonId) || nextLesson;
  const completedChapters = PANDAS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => PANDAS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return PANDAS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub pandas-hub">
      <div className="oops-hero pandas-hero">
        ...
        <div className="oops-hero-badge">PANDAS · PYTHON TRACK</div>
        ...
        <p className="oops-hero-sub">
          Learn Series, DataFrames, filtering, cleaning, groupby, and real
          tabular data workflows with hands-on Python challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${PANDAS_LESSONS.length} lessons · ${earnedXP}/${PANDAS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${PANDAS_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume Pandas" : "Start Pandas"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a Pandas topic</span>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{PANDAS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{PANDAS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{PANDAS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
